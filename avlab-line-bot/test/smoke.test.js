...
test('practice reuses real attendance cards and scoring without touching production records', () => {
  ...
  assert.doesNotMatch(graded.text, /https:\/\/forms\.gle\//);
  assert.equal(graded.quickReply.items.some(item => item.action.type === 'uri' && /forms\.gle/.test(item.action.uri)), false);
