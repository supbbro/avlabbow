...
test('practice reuses real attendance cards and scoring without touching production records', () => {
  const now = new Date();
  ...
  const commandFor = (reply, label) => reply.quickReply?.items.find(item => item.action.label === label)?.action.data;
  let firstCard = practiceMode.handleCommand('開始練習', first, now);
  assert.equal(firstCard.lineMessage.template.type, 'carousel');
  assert.deepEqual(firstCard.lineMessage.template.columns.map(column => column.title), ['陳小晴', '林小宇']);
  const arrival = firstCard.lineMessage.template.columns[0].actions[0].data;
  assert.match(arrival, /^練習 到場判定 PRACTICE-/);
  const taskId = arrival.split(' ')[2];
  const firstStudentId = arrival.split(' ')[3];
  let secondCard = practiceMode.handleCommand('開始練習', second, now);
  assert.deepEqual(secondCard.lineMessage.template.columns.map(column => column.title), ['陳小晴', '林小宇']);
  let graded = practiceMode.handleCommand(arrival, first, now);
  assert.match(graded.text, /出席：到場.*簡答題：⏳ 尚未評分/s);
  graded = practiceMode.handleCommand(commandFor(graded, '簡答題 ✅'), first, now);
  assert.match(graded.text, /簡答題：✅ 通過/);
  graded = practiceMode.handleCommand(commandFor(graded, '上機 ❌'), first, now);
  assert.match(graded.text, /上機未通過.*第一次補考上機考報名表/s);
  assert.match(graded.text, /https:\/\/forms\.gle\/3be87wRzRBKvdkFb6/);
  assert.match(graded.text, /請勿用假資料送出/);
  assert.ok(graded.quickReply.items.some(item => item.action.type === 'uri' && /forms\.gle\/3be87wRzRBKvdkFb6/.test(item.action.uri)));
  assert.match(practiceMode.handleCommand(`練習 查看考生 ${taskId} ${firstStudentId}`, second, now).text, /請使用練習卡片上的按鈕/);
  secondCard = practiceMode.handleCommand('練習點名', second, now);
  assert.match(secondCard.text, /陳小晴｜未點名/);
  const secondArrival = secondCard.lineMessage.template.columns[0].actions[0].data;
  let shortFailed = practiceMode.handleCommand(secondArrival, second, now);
  shortFailed = practiceMode.handleCommand(commandFor(shortFailed, '簡答題 ❌'), second, now);
  assert.match(shortFailed.text, /簡答題未通過：補考週.*不需填上機報名表/s);
  assert.doesNotMatch(shortFailed.text, /https:\/\/forms\.gle\//);
  assert.match(shortFailed.text, /簡答補考不需要上機報名表/);
  let student = practiceMode.handleCommand(`練習 查看考生 ${taskId} ${firstStudentId}`, first, now);
  assert.match(student.text, /上機：❌ 未通過/);
  const correction = practiceMode.handleCommand(commandFor(student, '修改紀錄'), first, now);
  assert.ok(commandFor(correction, '修正簡答題'));
  assert.ok(commandFor(correction, '修正上機考'));
  const correctionStep = practiceMode.handleCommand(commandFor(correction, '修正上機考'), first, now);
  assert.match(correctionStep.text, /只會修改上機考/);
  student = practiceMode.handleCommand(commandFor(correctionStep, '通過'), first, now);
  assert.match(student.text, /已更正上機：通過/);
  assert.match(student.text, /保證金：✅ 可退保證金/);
  assert.match(practiceMode.handleCommand('練習 近期任務', first, now).text, /點名首頁｜H6/);
  assert.match(practiceMode.handleCommand('練習重來', first, now).text, /陳小晴｜未點名/);
  assert.match(practiceMode.handleCommand('開始教學練習', first, now).lineMessage.template.columns[0].actions[0].label, /學生已到/);
  const retestCards = practiceMode.handleCommand('開始補考練習', first, now);
  let retest = practiceMode.handleCommand(retestCards.lineMessage.template.columns[0].actions[0].data, first, now);
  assert.match(retest.text, /簡答題：✅ 通過/);
  assert.ok(commandFor(retest, '上機 ❌'));
  assert.equal(commandFor(retest, '簡答題 ❌'), undefined);
  retest = practiceMode.handleCommand(commandFor(retest, '上機 ❌'), first, now);
  assert.match(retest.text, /第二次補考上機考報名表：https:\/\/forms\.gle\/t1vrm4U43xMhoWxD9/);
  assert.match(retest.text, /第二次補考須繳交 100 元，且不退費/);
  assert.ok(retest.quickReply.items.some(item => item.action.type === 'uri' && /forms\.gle\/t1vrm4U43xMhoWxD9/.test(item.action.uri)));
