...
- 09:00 起每 5 分鐘：檢查對內任務，在任務當週星期一 09:00 與任務當日 09:00 各提醒一次。
- 09:00 起每 5 分鐘：檢查已綁定的教學群組；週一 09:00 推播本週 1151 教學總排程，每天 09:00 推播當日排程。
- 每天 20:00：保留舊版明日任務提醒。
- 每週一 01:00：結算射龍門「賭神」。
- 每分鐘：檢查對外任務的一小時提醒。
- 每分鐘：依報名回覆的學號核對已綁定對外學生，推送一次報名成功通知。通知紀錄寫入「報名成功通知紀錄」分頁；首次啟用會將當時已綁定的舊報名建立基準，不追發舊通知。
- 2026/09/28 起：對尚未繳交、且已綁定 LINE 的報名學生發送一次保證金繳費提醒；之後仍保留截止日前一天及最早考試前一天的未繳提醒。同一天撞期會合併為一則。已繳交者不催繳。開始日預設為 `2026-09-28`，如需更改可設定 `EXTERNAL_DEPOSIT_REMINDER_START=YYYY-MM-DD`，不設定即可使用預設值。

對外學生可在報名前先輸入「我是 姓名 9 位數學號」綁定；報名後也可再綁定。報名成功通知只有報名回覆含有效考試項目，且學號、姓名與綁定資料相符（容許一字差異）時才發送；若姓名差異較大，須重新綁定。保證金表可作為報名後綁定的備援名單，但單有保證金紀錄不會觸發「報名成功」通知。對內助理仍須在現役名冊內才能綁定。
報名成功通知不包含應繳金額或繳費提醒；繳費通知由保證金流程依對帳表狀態另外發送。

LINE 訊息優先於等待中的背景同步；選身份畫面共用試算表快取，實際輸入「我是 姓名／學號」時名單最多快取 3 秒，避免大量使用者同時綁定造成 Google Sheets 讀取尖峰。每處理 10 個 LINE 工作會讓 1 個背景工作執行，避免提醒長時間被擠掉。同一種背景同步不會重複堆進佇列。試算表寫入仍一次執行一項工作，以免多人同時修改造成資料競爭。
...
群組採安靜模式：只會主動發送已設定的排程提醒；一般聊天、貼圖與個人功能關鍵字不會觸發回覆。群組內僅接受 `綁定教學群組 群組名稱`、`解除教學群組`、`今日教學排程`、`本週教學排程`。

排程提醒只顯示工作內容；文件智慧型方塊、附件檔名與網址都不顯示。
